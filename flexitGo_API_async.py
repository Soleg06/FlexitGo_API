...
class FlexitGo:
    ...
    async def getSensors(self):
        ...
        try:
            ...
            out["temps"]["home_air_temperature"] = await self._float_sensor(self.HOME_AIR_TEMPERATURE_PATH)
            out["temps"]["away_air_temperature"] = await self._float_sensor(self.AWAY_AIR_TEMPERATURE_PATH)
            out["temps"]["Uteluft"]  = await self._float_sensor(self.OUTSIDE_AIR_TEMPERATURE_PATH)
            out["temps"]["Tilluft"]  = await self._float_sensor(self.SUPPLY_AIR_TEMPERATURE_PATH)
            out["temps"]["Avluft"]  = await self._float_sensor(self.EXHAUST_AIR_TEMPERATURE_PATH)
            out["temps"]["Frånluft"]  = await self._float_sensor(self.EXTRACT_AIR_TEMPERATURE_PATH)
            out["temps"]["room_temperature"]  = await self._float_sensor(self.ROOM_TEMPERATURE_PATH)
            out["modes"]["electric_heater"]  = await self._bool_sensor(self.HEATER_PATH)
            out["modes"]["ventilation_mode"]  = await self._ventilation_mode(await self._int_sensor(self.MODE_PATH))
            out["modes"]["ventilation_mode_cal"]  = await self._ventilation_mode(await self._int_sensor(self.MODE_HOME_HIGH_CAL_PUT_PATH))
            out["modes"]["heat_exchanger_speed"] = await self._int_sensor(self.HEAT_EXCHANGER_SPEED_PATH)
            out["fläkt"]["supply_fan_speed"]  = await self._int_sensor(self.SUPPLY_FAN_SPEED_PATH)
            out["fläkt"]["supply_fan_control_signal"]  = await self._float_sensor(self.SUPPLY_FAN_CONTROL_SIGNAL_PATH)
            out["fläkt"]["extract_fan_speed"]  = await self._int_sensor(self.EXTRACT_FAN_SPEED_PATH)
            out["fläkt"]["extract_fan_control_signal"]  = await self._float_sensor(self.EXTRACT_FAN_CONTROL_SIGNAL_PATH)
            out["modes"]["additional_heater"]  = await self._bool_sensor(self.ADDITIONAL_HEATER_PATH)
            out["alarm"]["alarm_code_a"]  = await self._int_sensor(self.ALARM_CODE_A_PATH)
            out["alarm"]["alarm_code_b"]  = await self._int_sensor(self.ALARM_CODE_B_PATH)
            out["modes"]["calendar_temporary_override"]  = await self._bool_sensor(self.CALENDAR_TEMPORARY_OVERRIDE_PATH)
            out["modes"]["calendar_active"] = await self._calendar_active(self.MODE_HOME_HIGH_CAL_PUT_PATH)
            out["modes"]["boost_duration"] = await self._int_sensor(self.BOOST_DURATION_PATH)
            out["modes"]["away_delay"]  = await self._int_sensor(self.AWAY_DELAY_PATH)
            out["modes"]["fireplace_duration"]  = await self._int_sensor(self.FIREPLACE_DURATION_PATH)
    ...
    async def setPresetMode(self, presetMode):
        ...
        currentMode = await self.getSensors()["modes"]["ventilation_mode"]
        result = list()
